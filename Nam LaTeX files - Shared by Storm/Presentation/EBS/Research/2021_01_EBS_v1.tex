...
\begin{frame} 
	...
	%\begin{itemize}
	%\item  Wage gap between native (N) and foreign (F) workers has increased over time and is %concentrated near the tails of the wage distribution
	%\end{itemize}
	
	%\begin{itemize}
	%	\item Research using occupation-level task data emphasizes occupational segregation \textit{(Peri \& Sparber 2009, 2011; Haas et al. 2013)} 
	%	\item Yet, there has been a convergence in tasks among N \& F workers over time
	%\end{itemize}
...
\begin{frame} 
	...
	\begin{itemize}
		...
		%\item To address this question, the present paper explores two key hypotheses
		%\bigskip
		
		%	\begin{enumerate}
		%	\item N \& F workers with common occupation do \textit{not} perform the same set of tasks
		%%meaning there was a heterogenous adoption within abstract task category
		%	\medskip
		%	\item Even within occupations there is variation in interactive tasks which has become more important over time
		%as those activities are more important for workers near the top of the wage distribution
		%	\end{enumerate}
	\end{itemize}
...
%	\begin{itemize}
%		\item \textbf{\underline{Measuring individual task content}}: Compute relative importance of task category $j$\footnote[frame]{$j = 1$ (NR Analytic), $j = 2$ (NR Interactive), $j = 3$ (R Cognitive), $j = 4$ (R Manual), \\ $j = 5$ (NR Manual)}  for individual $i$ at time $t$  \\ \smallskip (following \textit{Antonczyk, Fitzenberger \& Leuschner 2009})
		%(following \textit{Antonczyk, Fitzenberger, and Leuschner 2009})
%	\end{itemize}
...
\begin{frame}[label=wi_spec]
	...
	%	\begin{itemize} 
	%		\item $IFEV_{NRI}^{0.8} (1992-99) = 0.25$ $\Longrightarrow$ $IFEV_{NRI}^{0.8} (2006-18) = 0.33$
	%	\end{itemize} 
%\hyperlink{regression}{\beamergotobutton{Regression}}
